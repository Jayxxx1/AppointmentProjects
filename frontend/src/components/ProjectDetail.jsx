...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setError(e?.response?.data?.message || "โหลดข้อมูลโปรเจกต์ไม่สำเร็จ");
      } finally {
  ...
  const canEdit = useMemo(
    () => currentRole === "admin" || String(createdById) === String(currentUserId),
    [currentRole, createdById, currentUserId]
  );
  ...
  const searchAdvisor = async () => {
    ...
    try {
      ...
      const rs = await projectService.searchUsers(advisorQuery.trim(), "teacher", 8);
  ...
  const searchMembers = async () => {
    ...
    try {
      ...
      const rs = await projectService.searchUsers(memberQuery.trim(), "student", 8);
      // กรองออกคนที่อยู่แล้ว/ที่เป็น advisor
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <div className="bg-white/80 backdrop-blur p-6 rounded-2xl shadow-xl text-center">
          <div className="mb-4 font-semibold text-gray-700">ไม่พบโปรเจกต์</div>
          <Link
            to="/projects"
            className="inline-flex items-center px-4 py-2 rounded-xl bg-gray-100 hover:bg-gray-200"
          >
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center">
        <div className="bg-white/80 backdrop-blur p-8 rounded-2xl shadow-xl text-center border border-red-100">
          ...
          <div className="text-gray-600 mb-6">
            เฉพาะผู้สร้างโปรเจคหรือผู้ดูแลระบบเท่านั้นที่สามารถแก้ไขโปรเจคได้
          </div>
          <Link
            to="/projects"
            className="inline-flex items-center px-4 py-2 rounded-xl bg-gray-100 hover:bg-gray-200"
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 overflow-hidden">
      ...
      <div className="relative z-10 p-6 lg:p-8">
        {!editMode ? (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={deleting}
                      className={`shadow-xl text-white px-6 py-3 rounded-full font-medium transition-all duration-300 backdrop-blur-sm flex items-center space-x-2 transform hover:scale-105 ${
                        deleting
                          ? "bg-red-400 shadow-red-400/40 cursor-not-allowed"
                          : "bg-red-500 hover:bg-red-400 shadow-red-500/50 hover:shadow-red-400/50"
                      }`}
                      title="ลบโปรเจค"
                    >
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-100">
                    ...
                    <p className="text-gray-800 font-semibold">
                      {project.advisor?.fullName || project.advisor?.email || "—"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 p-6">
                ...
                <div className="space-y-3">
                  ...
                  {!(project.members || []).length && (
                    <div className="text-gray-500">— ไม่มีสมาชิก —</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 p-6">
              <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                <svg className="w-6 h-6 mr-2 text-pink-600" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M8 12a1 1 0 001-1V5a1 1 0 112 0v6a3 3 0 11-3-3 1 1 0 110 2 1 1 0 100 2zM5 8a5 5 0 1110 0v4a5 5 0 11-10 0V8z" />
                </svg>
                ไฟล์แนบของโปรเจ็กต์
              </h3>
                    ...
                    return (
                      <li key={a._id} className="py-3 flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="px-3 py-2 text-sm rounded-lg bg-gradient-to-r from-gray-100 to-gray-200 hover:from-blue-500 hover:to-purple-500 hover:text-white transition-colors"
                        >
                          ดาวน์โหลด
                        </button>
                      </li>
          ...
          // -------- edit form (คงสไตล์เดิม, เพิ่มเฉพาะส่วนที่จำเป็น) --------
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      value={
                        ...
                        project?.advisor?.username ||
                        project?.advisor?.email ||
                        ""
                      }
                      onChange={(e) => setAdvisorQuery(e.target.value)}
                      placeholder="ค้นหาอาจารย์ด้วยชื่อ/อีเมล"
                      className="flex-1 rounded-xl border border-gray-200 px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    />
                    <button
                      type="button"
                      onClick={searchAdvisor}
                      className="px-4 py-3 rounded-xl bg-gray-100 hover:bg-gray-200"
                    >
                      ค้นหา
                    </button>
                  </div>
                  {advisorLoading && <div className="text-sm text-gray-500">กำลังค้นหา...</div>}
                  ...
                  <div className="pt-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="ค้นหานักศึกษาด้วยชื่อ/อีเมล/รหัส"
                        className="flex-1 rounded-xl border border-gray-200 px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
                      <button
                        type="button"
                    ...
                    {memberLoading && <div className="text-sm text-gray-500 mt-1">กำลังค้นหา...</div>}
                    {!!memberResult.length && (
                    ...
                    {(attachments || []).map((a) => (
                      <div key={a._id} className="border border-gray-200 rounded-xl p-3 flex items-center justify-between">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-gray-500">
                            {a.mimeType || "ไฟล์"} • {a.size ? `${Math.max(1, Math.round(a.size / 1024))} KB` : "-"}
                          </div>
                        </div>
                        <button
                          type="button"
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    type="submit"
                    disabled={saving}
                    className={`flex-1 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-6 py-3 rounded-xl font-medium transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 ${
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setForm({
                        ...
                        advisorId: project?.advisor?._id || project?.advisor || "",
                      });
                    }}
                    className="flex-1 bg-gradient-to-r from-gray-400 to-gray-500 hover:from-gray-500 hover:to-gray-600 text-white px-6 py-3 rounded-xl font-medium transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                  >
                    ยกเลิก
                  </button>
                </div>
