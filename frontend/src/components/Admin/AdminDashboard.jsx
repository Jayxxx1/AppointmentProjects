...
import { Loader2, CalendarClock, CheckCircle2, Clock, XCircle, RefreshCw, ListChecks, AlertTriangle, Filter, Search, FireExtinguisher } from "lucide-react";
import AppointmentChart from "./AppointmentChart";
import { MdFireExtinguisher, MdOutline10K, MdOutlineSummarize } from "react-icons/md";
...
export default function AdminDashboard() {
  ...
  // Helpers
  const toDate = (it) => {
    // Backend commonly returns a single ISO date field `startAt` (Date) for appointments.
    if (!it) return null;
    if (it.startAt) {
      try {
        return new Date(it.startAt);
      } catch {
        // fallthrough
      }
    }
    // Many older docs store separate date (YYYY-MM-DD) + startTime (HH:mm)
    const d = it?.date || it?.startDate || it?.scheduledDate; // fallbacks
    const s = it?.startTime || "00:00";
    if (!d) return it?.createdAt ? new Date(it.createdAt) : null;
  ...
  const statusLabel = (s) => {
    switch (s) {
      ...
      case "completed": return { text: "เสร็จสิ้น", color: "bg-blue-50 text-blue-700 border-blue-200" };
  ...
  // Remove appointment locally after successful admin delete
  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <MetricCard icon={CalendarClock} label="นัดหมายทั้งหมด" value={metrics.total} color="gray" />
        ...
        <MetricCard icon={MdOutlineSummarize } label="เสร็จสิ้นและสรุปบันทึก" value={metrics.completed} color="blue" />
      ...
      {/* Appointment chart (replaces upcoming list) */}
      <div className="mb-6">
        <AppointmentChart items={items} days={30} />
      </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-gray-900 flex items-center gap-2">
              ...
              รายการนัดหมายบางส่วน
            </h2>
            <div className="text-sm text-gray-500">
              แสดง {Math.min(filtered.length, 10)} / {items.length} รายการ
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <Th>วัน-เวลา</Th>
                <Th>หัวข้อ</Th>
                <Th className="hidden md:table-cell">โปรเจค</Th>
                <Th>สถานะ</Th>
            ...
            <tbody className="divide-y divide-gray-200">
              ...
              {!loading && !error && filtered.slice(0, 10).map((it) => (
                <Row key={it._id || it.id} it={it} toDate={toDate} statusLabel={statusLabel} onDeleted={handleRemoveLocal} />
              ))}
            </tbody>
          </table>
        </div>
...
function Row({ it, toDate, statusLabel, onDeleted }) {
  const dt = toDate(it);
  const dateStr = dt ? dt.toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }) : "-";
  const s = statusLabel(it.status);
  // Backend populates `createBy` for the appointment creator.
  const creatorName = it?.createBy?.fullName || it?.createBy?.username || it?.createBy?.email || "-";
  ...
  return (
    <tr>
      ...
      <td className="px-6 py-4 hidden md:table-cell text-gray-900">{it.location || it.project?.name || "-"}</td>
      <td className="px-6 py-4">
        <span className={`inline-flex items-center px-2 py-1 rounded-md border text-xs font-medium ${s.color}`}>
          {s.text}
        </span>
      ...
      <td className="px-6 py-4 text-right">
        <div className="flex items-center justify-end gap-4">
          ...
          <DeleteButton id={it._id || it.id} onDeleted={onDeleted} />
        </div>
      </td>
    </tr>
  );
...
function DeleteButton({ id, onDeleted }) {
  ...
  const handle = async () => {
    if (!confirm('ยืนยัน: ลบการนัดหมายนี้ถาวร? กระทำนี้ไม่สามารถย้อนกลับได้.')) return;
    try {
      setLoading(true);
  ...
  return (
    <button onClick={handle} disabled={loading} className="text-sm text-red-600 font-medium">
      {loading ? 'กำลังลบ...' : 'ลบ'}
    </button>
  );
}
