...
const RescheduleModal = ({ isOpen, onClose, onSubmit, appointment }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (en <= s) { setError('เวลาสิ้นสุดต้องหลังเวลาเริ่ม'); return; }
      if ((en - s) > 1000 * 60 * 60) { setError('นัดได้ไม่เกิน 1 ชั่วโมงต่อครั้ง'); return; }
      if ((en - s) < 20 * 60 * 1000) { setError('นัดหมายต้องมีระยะเวลาอย่างน้อย 20 นาที'); return; }
