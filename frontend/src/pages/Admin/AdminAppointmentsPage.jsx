...
import { CalendarClock, RefreshCw, Search, Filter, Trash2 } from 'lucide-react';
...
export default function AdminAppointmentsPage() {
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm('คุณแน่ใจว่าจะลบนัดหมายนี้? การลบถาวรไม่สามารถย้อนกลับได้');
    if (!ok) return;
    try {
      setDeletingId(id);
      setError('');
                  ...
                  <tr key={it._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <Td className="text-right space-x-3">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm bg-rose-50 text-rose-600 hover:bg-rose-100 disabled:opacity-60"
                        title="ลบ"
                      >
                        {deletingId === it._id ? (
