...
export default function CreateProject() {
  ...
  // If the current user is a student, block access to this page when they already have a project.
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        if (role !== 'student') return;
        // ask server for this user's projects
        const mine = await projectService.listMine();
        if (!alive) return;
        const hasProject = Array.isArray(mine) ? mine.length > 0 : (mine?.length > 0);
        if (hasProject) {
          // redirect back to projects list
          navigate('/projects', { replace: true });
        }
      } catch (e) {
        // ignore errors here — allow the page to render; server will block create if necessary
      }
    })();
    return () => { alive = false; };
  }, [role]);
