                ...
                <div key={it._id} className="border rounded-lg p-4 bg-white shadow-sm">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {it.project?.advisor && (
                        <div className="text-sm text-gray-600">อาจารย์ที่ปรึกษา: {it.project?.advisor?.fullName || it.project?.advisor?.username || it.project?.advisor?.email}</div>
                      )}
                    </div>
                    <div className="text-right flex flex-col items-end gap-2">
                      {it.appointment && (
                        <Link to={`/appointments/${it.appointment._id || it.appointment}`} className="text-blue-600 hover:underline">ดูนัดหมาย</Link>
                      )}
                      <Link to={`/meetsummary/${it._id}`} className="text-blue-600 hover:underline">ดูสรุป</Link>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-end gap-2">
                    {(user?.role === 'teacher' || user?.role === 'admin') && (
                      <button onClick={() => setOpenScheduleFor(it)} className="px-3 py-2 bg-blue-500 text-white rounded-lg">นัดหมายต่อจากนัดหมายครั้งนี้</button>
                    )}
                  </div>
                </div>
              ))}
