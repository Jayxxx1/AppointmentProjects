...
export default function CreateAppointment() {
  ...
  // ✅ helper: วันที่วันนี้แบบ local (ไม่โดน UTC ลบวัน)
  ...
  useEffect(() => {
    ...
    setSelectedAdvisor(
      proj?.advisor
        ? {
            name: proj.advisor.fullName || proj.advisor.username || proj.advisor.email,
  ...
  const validateStep = (step) => {
    ...
    if (step === 3) {
      ...
      if (formData.startTime && formData.endTime) {
        ...
        if ((end - start) > 1000 * 60 * 60) newErrors.endTime = "นัดได้ไม่เกิน 1 ชั่วโมงต่อครั้ง";
      }
  ...
  const handlePreview = (e) => {
    ...
    (async () => {
      try {
        const av = await appointmentService.checkAvailability({ project: formData.project, date: formData.date, startTime: formData.startTime, endTime: formData.endTime });
        if (!av.available) {
          showPopupError('ช่วงเวลานี้มีการนัดหมายอื่นอยู่แล้ว');
          return;
        }
        setShowPreview(true);
      } catch (err) {
        // if check fails, allow preview but show warning
        setShowPreview(true);
      }
    })();
  };
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      // ส่งเมลไปที่อาจารย์อัตโนมัติ (ถ้ามี)
      if (selectedAdvisor?.email) {
        payload.participantEmails = [selectedAdvisor.email];
      }

      // Before submission, double-check availability to avoid race conditions
      try {
        const av2 = await appointmentService.checkAvailability({ project: formData.project, date: formData.date, startTime: formData.startTime, endTime: formData.endTime });
        if (!av2.available) {
      ...
      // Prefer a clear conflict message when backend indicates overlap (409)
      let msg = "สร้างนัดหมายไม่สำเร็จ";
      try {
        const status = err?.response?.status;
        const bodyMsg = err?.response?.data?.message || '';
        if (status === 409 || /ช่วงเวลา|ถูกจอง|มีการนัดหมายอื่น/.test(String(bodyMsg))) {
          msg = 'ช่วงเวลาที่เลือกถูกจองแล้ว';
        } else if (bodyMsg) {
          msg = bodyMsg;
        }
      } catch (e) {}
