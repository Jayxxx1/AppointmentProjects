...
export const runReminderWorker = async () => {
  ...
  try {
    ...
    if (expiredFollowUps && expiredFollowUps.length > 0) {
      console.log(
        `Found ${expiredFollowUps.length} expired follow‑up(s). Marking as cancelled.`,
      );
      for (const ap of expiredFollowUps) {
        try {
          ...
          ap.reason =
            ap.reason ||
            'Follow‑up expired (no student approval within allowed window)';
          await ap.save();
          ...
          if (allEmails.length > 0) {
            ...
            await sendEmail({
              to: allEmails,
              subject: `[แจ้งเตือน] นัดหมายหมดเวลา: ${ap.title}`,
              html,
            });
          }
        } catch (e) {
          console.error(
            'Failed to mark expired follow‑up as cancelled for',
            ap._id,
            e,
          );
        }
    ...
    for (const appointment of appointmentsToRemind) {
      try {
        ...
        if (allEmails.length > 0) {
          ...
          console.log(
            `Reminder sent for appointment: ${appointment.title}`,
          );
        }
      } catch (emailError) {
        console.error(
